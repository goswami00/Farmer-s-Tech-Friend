...
def detect_plant_disease(img):
    # Disable scientific notation for clarity
    np.set_printoptions(suppress=True)

    # Load the model
    model = load_model("./Plants_disease_classify.h5", compile=False)
...
if input_img is not None:
    if st.button("Detect"):
        ...
        with col2:
            ...
            label, confidence_score = detect_plant_disease(image_file)
            st.success(label)
